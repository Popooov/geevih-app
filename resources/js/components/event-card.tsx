...
function EventBadge({ children, variant = 'default' }: { children: React.ReactNode; variant?: 'default' | 'secondary' | 'live' | 'finished' }) {
    const styles = {
        default: 'bg-primary/10 text-primary dark:bg-primary/15 dark:text-primary-foreground',
...
export default function EventCard({
    ...
    return (
        <article
            className={cn(
                'group flex h-full flex-col overflow-hidden rounded-[2rem] transition-all duration-300',
                isPast
            ...
            <div className="relative aspect-[16/9] overflow-hidden bg-muted dark:bg-zinc-900">
                <img
                    ...
                    className={cn(
                        ...
                        isPast ? 'brightness-[0.86] grayscale-[0.75]' : 'group-hover:scale-[1.025]',
                    )}
                />
